import Product from "../Product/Product";
import "./productlist.css";

function ProductList()
{
    return(
    <>
    <div className="productlist">
        <Product 
        price={1222250} 
        name={"Trimmer"} 
        img={"https://rukminim2.flixcart.com/image/612/612/xif0q/shaver/t/u/z/new-shaver-for-trimmer-hair-cutting-saving-classic-machine-beard-original-imagsxg2gwpme8u2.jpeg?q=70"}
        desc={"Trimmer ladies set"}
        />
        <Product 
        price={5500} 
        name={"Lenova Monitor"} 
        img={"https://rukminim2.flixcart.com/image/312/312/ko8xtow0/monitor/t/a/y/d24-20-66aekac1in-lenovo-original-imag2qwzazcdmqtb.jpeg?q=70"}
        desc={"Monitor lenova 1080p"}
        />
         <Product 
        price={5500} 
        name={"watch"} 
        img={"https://rukminim2.flixcart.com/image/612/612/xif0q/watch/3/h/m/-original-imagqhvfh2fpywqf.jpeg?q=70"}
        desc={"Monitor lenova 1080p"}
        />
        <Product 
        price={150} 
        name={"Mobile Phone"} 
        img={"https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcQgLHZSN1zBG7svjyp4H047FXO-24566CxTZVfSSdh7JV1vXdAF3WuoogK59d97-4VGxUDqch4zjcUdnaHPe967Whzg0DqhXvQSy7fj7ZZ-nHdznrWdx1Y_Mg"}
        desc={"Trimmer ladies set"}
        />
        <Product 
        price={5500} 
        name={"air pods"} 
        img={"https://store.storeimages.cdn-apple.com/4668/as-images.apple.com/is/airpods-4-anc-select-202409_FV1?wid=976&hei=916&fmt=jpeg&qlt=90&.v=1725502554118"}
        desc={"Monitor lenova 1080p"}
        />
         <Product 
        price={5500} 
        name={"Appleipads"} 
        img={"https://static-assets-web.flixcart.com/www/linchpin/batman-returns/images/fk-default-image-75ff340b.png?q=90"}
        desc={"Monitor lenova 1080p"}
        />
        <Product 
        price={150} 
        name={"Trimmer"} 
        img={"https://rukminim2.flixcart.com/image/612/612/xif0q/shaver/t/u/z/new-shaver-for-trimmer-hair-cutting-saving-classic-machine-beard-original-imagsxg2gwpme8u2.jpeg?q=70"}
        desc={"Trimmer ladies set"}
        />
        <Product 
        price={5500} 
        name={"Lenova Monitor"} 
        img={"https://rukminim2.flixcart.com/image/312/312/ko8xtow0/monitor/t/a/y/d24-20-66aekac1in-lenovo-original-imag2qwzazcdmqtb.jpeg?q=70"}
        desc={"Monitor lenova 1080p"}
        />
         <Product 
        price={5500} 
        name={"Lenova Monitor"} 
        img={"https://rukminim2.flixcart.com/image/312/312/ko8xtow0/monitor/t/a/y/d24-20-66aekac1in-lenovo-original-imag2qwzazcdmqtb.jpeg?q=70"}
        desc={"Monitor lenova 1080p"}
        />
        <Product 
        price={150} 
        name={"Trimmer"} 
        img={"https://rukminim2.flixcart.com/image/612/612/xif0q/shaver/t/u/z/new-shaver-for-trimmer-hair-cutting-saving-classic-machine-beard-original-imagsxg2gwpme8u2.jpeg?q=70"}
        desc={"Trimmer ladies set"}
        />
        <Product 
        price={150} 
        name={"Trimmer"} 
        img={"https://rukminim2.flixcart.com/image/612/612/xif0q/shaver/t/u/z/new-shaver-for-trimmer-hair-cutting-saving-classic-machine-beard-original-imagsxg2gwpme8u2.jpeg?q=70"}
        desc={"Trimmer ladies set"}
        />
        <Product 
        price={150} 
        name={"Trimmer"} 
        img={"https://rukminim2.flixcart.com/image/612/612/xif0q/shaver/t/u/z/new-shaver-for-trimmer-hair-cutting-saving-classic-machine-beard-original-imagsxg2gwpme8u2.jpeg?q=70"}
        desc={"Trimmer ladies set"}
        />
    </div>
    </>
    );

}
export default ProductList;